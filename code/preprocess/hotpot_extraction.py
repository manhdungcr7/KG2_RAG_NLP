...
def extract_triplets(llm, ctx):
    ...
    try:
        ...
        print(f"\n[Cảnh báo] Lỗi Timeout hoặc LLM từ chối trả lời. Bỏ qua câu này. Chi tiết: {e}")
        return [] # Trả về mảng rỗng để hệ thống bỏ qua và làm câu tiếp theo
    triplets = set()
...
# --- NÂNG CẤP TÍNH NĂNG RESUME ---
# Quét xem trên ổ cứng đã có sẵn bao nhiêu file hoàn thành để báo cáo cho người dùng
existing_files = [f for f in os.listdir(out_dir) if f.endswith('.json')]
print(f"[*] Đã tìm thấy {len(existing_files)} thực thể (entities) được xử lý từ trước.")
print("[*] Hệ thống sẽ tua nhanh (fast-forward) qua các dữ liệu này...\n")

count = 0

for sample in tqdm(data, desc="Tiến trình duyệt Dataset"):
    ...
    for ctx in ctxs:
        ...
        # Làm sạch tên file, thay thế tất cả các ký tự cấm của Windows thành dấu gạch dưới
        safe_ent = re.sub(r'[<>:"/\\|?*]', '_', ent)
        out_path = os.path.join(out_dir, f'{safe_ent}.json')
        
        # --- XỬ LÝ LỖI DO CTRL+C ---
        if os.path.exists(out_path):
            # Nếu file tồn tại và có dữ liệu (> 0 byte) -> Bỏ qua an toàn.
            if os.path.getsize(out_path) > 0:
                continue
            else:
                # Nếu file 0 byte do lần trước ngắt ngang lúc đang lưu -> Xóa đi làm lại.
                os.remove(out_path)
